...
public class ReadLetterFirstThreeWordsFromDocToExcel {
    ...
    public static void main(String[] args) {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            header.createCell(1).setCellValue("Heading");

            int rowNum = 1;

            for (File file : folder.listFiles()) {
                if (file.isFile() &&(file.getName().startsWith("letterAnswer")) &&(file.getName().endsWith(".doc") || file.getName().endsWith(".docx"))) {
    ...
    private static String extractFirstThreeWords(File file) {
        try {
            String text = "";
            if( (file.getName().endsWith(".docx"))&&(file.getName().startsWith("letterAnswer"))) {
            ...
            } else if ((file.getName().endsWith(".doc"))&&(file.getName().startsWith("letterAnswer"))) {
                try (FileInputStream fis = new FileInputStream(file);
