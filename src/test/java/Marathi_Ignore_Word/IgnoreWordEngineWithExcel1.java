...
public class IgnoreWordEngineWithExcel1 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        File studentFolder = new File(
                "C:\\Users\\User\\Desktop\\New folder\\New folder\\New folder"
        );
        ...
        //  OUTPUT RESULT FILE
        String outputPath =
                "C:\\Users\\User\\Desktop\\New folder\\IgnoreWord_Result12.xlsx";
...
// ===============================
    private static List<String> checkIgnoreWords(
        ...
        // Convert student text to word set (FAST search)
        ...
        for (Row row : sheet) {
            ...
            // Column 2 = Question Paper Word (Model Word)
            String paperWord = normalize(getCellValue(row.getCell(2)));

            // Column 3 = Ignore Student Word
            String ignoreStudentWord = normalize(getCellValue(row.getCell(3)));

            if (paperWord.isEmpty() || ignoreStudentWord.isEmpty())
                continue;

            // Step 1: student word must exist in student file
            if (studentWordSet.contains(ignoreStudentWord)) {

                // Step 2: model word must NOT exist in student file
                if (!studentWordSet.contains(paperWord)) {

                    // Step 3: if both same → not ignore
                    if (!ignoreStudentWord.equals(paperWord)) {

                        matchedWords.add(ignoreStudentWord);
                    }
                }
            }
        }
