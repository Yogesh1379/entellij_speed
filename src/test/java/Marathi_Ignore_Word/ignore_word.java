...
import Final_SpeedMarking.Marathi_EnterAndTabCount3_lcs_diff;
...
public class ignore_word {
    ...
    public static void main(String[] args) throws IOException {
        //Batch wise allocation file
String batchWiseAllocation="F:\\GCC  TBC December 2025\\Question\\allocation\\marathi\\SUbjectiveMarathiHin.xlsx";
//        Ignore word excel file path
        String ignoreWordFile="F:\\GCC  TBC December 2025\\Marathi Ignore Word\\GCC TBC Mar30_40 Hindi 30_40 Speed ignore word_Final - Copy.xlsx";

// Folder containing student files
        File studentFolder = new File("C:\\Users\\User\\Desktop\\New folder");
        ...
        FileInputStream fisAlloc = new FileInputStream(batchWiseAllocation );
        ...
        for (File studentFile : docxFiles) {
            ...
            HashSet<String> paperSet = new HashSet<>();
            ...
            FileInputStream fisign = new FileInputStream(ignoreWordFile );
            Workbook ignWorkbook = new XSSFWorkbook(fisign);
            String ignSheet="";
                        switch (course1) {
                case 3 -> ignSheet="Marathi 30";
                case 4 -> ignSheet="Marathi 40";
                case 5 -> ignSheet="Hindi 30";
                case 6 -> ignSheet="Hindi 40";
            }
            Sheet sheetign = ignWorkbook.getSheet(ignSheet);
                     int rc=0;

                     for (Row row : sheetign) {
                         if(rc++==0) continue;
                        Cell batch= row.getCell(1);
                         Cell paperWord= row.getCell(2);
                         Cell stdWord= row.getCell(3);
                       String batchIGN=  getCellValueAsString(batch);
                        String paperWordIGN= getCellValueAsString(paperWord);
                        String stdWordIGN= getCellValueAsString(stdWord);

                         if (batchIGN.equalsIgnoreCase(batchname)) {
                             if (!paperWordIGN.isEmpty())
                                 paperSet.add(paperWordIGN.trim());

                             if (!stdWordIGN.isEmpty())
                                 StdSet.add(stdWordIGN.trim());
                         }

                     }

            System.out.println("Seat No: " + seatno);
            System.out.println("Paper Ignore Words: " + paperSet);
            System.out.println("Student Ignore Words: " + StdSet);
            ...
            for(String stdwords: StdSet) {
                if(stdwords.contains(studentText)) {

                    Row row = summarySheet.createRow(rowNum++);
                    row.createCell(0).setCellValue(seatno); // seat number from filename
                    row.createCell(1).setCellValue(stdfile);
                    row.createCell(2).setCellValue(stdwords);
                    System.out.println(stdwords+" std ignore word");
                }
            }
            ...
            Marathi_EnterAndTabCount3_lcs_diff.AlignmentResult align = computeAlignment(mWords, sWords);
            ...
            for (Marathi_EnterAndTabCount3_lcs_diff.AlignedToken token : align.alignedTokens) {
                XWPFRun r = para.createRun();
                if (token.type == Marathi_EnterAndTabCount3_lcs_diff.TokenType.MATCH) {
                    r.setText(token.student != null ? token.student + " " : token.model + " ");
                } else if (token.type == Marathi_EnterAndTabCount3_lcs_diff.TokenType.MISSING) {
                ...
                } else if (token.type == Marathi_EnterAndTabCount3_lcs_diff.TokenType.EXTRA) {
                ...
                } else if (token.type == Marathi_EnterAndTabCount3_lcs_diff.TokenType.SUBSTITUTION) {
    ...
    private static Marathi_EnterAndTabCount3_lcs_diff.AlignmentResult computeAlignment(String[] modelWords, String[] studentWords) {
        ...
        List<Marathi_EnterAndTabCount3_lcs_diff.AlignedToken> aligned = new ArrayList<>();
        int mi = 0, si = 0;
        ...
        while (mi < n || si < m) {
            ...
            if (modelSegmentLen > 0 || studentSegmentLen > 0) {
                ...
                for (int p = 0; p < pairLen; p++) {
                    ...
                    if (normalizeWord(mw).equals(normalizeWord(sw))) {
                        aligned.add(Marathi_EnterAndTabCount3_lcs_diff.AlignedToken.match(mw, sw));
                    } else {
                        aligned.add(Marathi_EnterAndTabCount3_lcs_diff.AlignedToken.substitution(mw, sw));
                    }
                }
                // Remaining model words (if any) are missing
                if (modelSegmentLen > pairLen) {
                    aligned.add(Marathi_EnterAndTabCount3_lcs_diff.AlignedToken.missing(Arrays.copyOfRange(modelWords, mi + pairLen, nextM)));
                }
                // Remaining student words (if any) are extra
                if (studentSegmentLen > pairLen) {
                    aligned.add(Marathi_EnterAndTabCount3_lcs_diff.AlignedToken.extra(Arrays.copyOfRange(studentWords, si + pairLen, nextS)));
                }
            ...
            if (k < mPos.size()) {
                ...
                if (mi < n) {
                    aligned.add(Marathi_EnterAndTabCount3_lcs_diff.AlignedToken.missing(Arrays.copyOfRange(modelWords, mi, n)));
                    mi = n;
                }
                if (si < m) {
                    aligned.add(Marathi_EnterAndTabCount3_lcs_diff.AlignedToken.extra(Arrays.copyOfRange(studentWords, si, m)));
                    si = m;
                }
            }
        }

        return new Marathi_EnterAndTabCount3_lcs_diff.AlignmentResult(aligned);
    }

    private enum TokenType { MATCH, MISSING, EXTRA, SUBSTITUTION }

    private static class AlignedToken {
        Marathi_EnterAndTabCount3_lcs_diff.TokenType type;
        ...
        static Marathi_EnterAndTabCount3_lcs_diff.AlignedToken match(String m, String s) {
            Marathi_EnterAndTabCount3_lcs_diff.AlignedToken t = new Marathi_EnterAndTabCount3_lcs_diff.AlignedToken();
            t.type = Marathi_EnterAndTabCount3_lcs_diff.TokenType.MATCH;
            t.model = m; t.student = s;
            return t;
        }

        static Marathi_EnterAndTabCount3_lcs_diff.AlignedToken missing(String[] seq) {
            Marathi_EnterAndTabCount3_lcs_diff.AlignedToken t = new Marathi_EnterAndTabCount3_lcs_diff.AlignedToken();
            t.type = Marathi_EnterAndTabCount3_lcs_diff.TokenType.MISSING;
            t.modelSequence = Arrays.asList(seq);
            return t;
        }

        static Marathi_EnterAndTabCount3_lcs_diff.AlignedToken extra(String[] seq) {
            Marathi_EnterAndTabCount3_lcs_diff.AlignedToken t = new Marathi_EnterAndTabCount3_lcs_diff.AlignedToken();
            t.type = Marathi_EnterAndTabCount3_lcs_diff.TokenType.EXTRA;
            t.studentSequence = Arrays.asList(seq);
            return t;
        }

        static Marathi_EnterAndTabCount3_lcs_diff.AlignedToken substitution(String m, String s) {
            Marathi_EnterAndTabCount3_lcs_diff.AlignedToken t = new Marathi_EnterAndTabCount3_lcs_diff.AlignedToken();
            t.type = Marathi_EnterAndTabCount3_lcs_diff.TokenType.SUBSTITUTION;
            t.model = m; t.student = s;
            return t;
        }
    }
    ...
    private static class AlignmentResult {
        List<Marathi_EnterAndTabCount3_lcs_diff.AlignedToken> alignedTokens;
        AlignmentResult(List<Marathi_EnterAndTabCount3_lcs_diff.AlignedToken> a) { this.alignedTokens = a; }
    }
