...
public class finalMarking {
            ...
            while (i < mWords.length || j < sWords.length) {
                ...
                if (refWord != null && stuWord != null && refWord.equals(stuWord)) {
                    ...
                    if (j + 1 < sWords.length && refWord.equals(sWords[j + 1])) {
                        ...
                        r.setColor("FF0000");
                        wrongWordCount++;
                        allMistakes.append("[ Wrong: ").append(stuWord).append(" | ").append(refWord).append(" ]; ");
                        i++; j++;
                    }
                } else {
                    if (refWord != null) {
    ...
    // ✅ UPDATED LOGIC — ignores leading/trailing spaces/tabs/enters
    private static String extractFullText(File file) throws IOException {
        ...
        // 🔹 Remove trailing blank paragraphs (ignore Enters at end)
        int lastNonEmptyIndex = -1;
        for (int i = doc.getParagraphs().size() - 1; i >= 0; i--) {
            if (!doc.getParagraphs().get(i).getText().trim().isEmpty()) {
                lastNonEmptyIndex = i;
                break;
            }
        }
        ...
        // 🔹 Process only up to last non-empty paragraph
        for (int i = 0; i <= lastNonEmptyIndex; i++) {
            ...
            if (text.trim().isEmpty()) {
                ...
                if (continuousBlankParas >= 2) maxBlankSequence++; // 2+ blank lines = extra enter
            } else {
                continuousBlankParas = 0;
            }

            // Ignore leading & trailing spaces/tabs
            String cleaned = text.replaceAll("^[\\t ]+", "").replaceAll("[\\t ]+$", "");

            // Count extra tabs/spaces only inside content
            totalTabCount += countMatches(cleaned, "\t{1,}");
            totalSpaceCount += countMatches(cleaned, " {3,}");
